...
--
-- Host: localhost:3306
-- Generation Time: Apr 28, 2021 at 12:34 AM
...
--
-- Stand-in structure for view `ClassesTakenByStudent`
-- (See below for the actual view)
--
CREATE TABLE `ClassesTakenByStudent` (
`FNAME` varchar(50)
,`LNAME` varchar(50)
,`EMAIL` varchar(50)
,`CLASS_ID` varchar(50)
...
--
-- Stand-in structure for view `DeansList`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `DoubleMajors`
-- (See below for the actual view)
--
CREATE TABLE `DoubleMajors` (
`FNAME` varchar(50)
,`LNAME` varchar(50)
...
--
-- Stand-in structure for view `GraduateInfo`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `StudentAdvisor`
-- (See below for the actual view)
--
...
,`LNAME` varchar(50)
,`GPA` varchar(50)
,`ADVISOR_FNAME` varchar(50)
,`ADVISOR_LNAME` varchar(50)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `StudentInternships`
-- (See below for the actual view)
--
...
,`INTERNSHIP CITY` varchar(50)
,`INTERNSHIP STATE` varchar(50)
);
...
--
-- Structure for view `ClassesTakenByStudent`
--
DROP TABLE IF EXISTS `ClassesTakenByStudent`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `ClassesTakenByStudent`  AS  select `STUDENT`.`FNAME` AS `FNAME`,`STUDENT`.`LNAME` AS `LNAME`,`STUDENT`.`EMAIL` AS `EMAIL`,`STUDENT_CLASS`.`CLASS_ID` AS `CLASS_ID` from (`STUDENT` join `STUDENT_CLASS` on(`STUDENT_CLASS`.`STUDENT_ID` = `STUDENT`.`STUDENT_ID`)) order by `STUDENT_CLASS`.`CLASS_ID` ;

-- --------------------------------------------------------

--
-- Structure for view `DeansList`
--
DROP TABLE IF EXISTS `DeansList`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `DeansList`  AS  select `STUDENT`.`FNAME` AS `First Name`,`STUDENT`.`LNAME` AS `Last Name` from `STUDENT` where `STUDENT`.`GPA` > 3.5 ;

-- --------------------------------------------------------

--
-- Structure for view `DoubleMajors`
--
DROP TABLE IF EXISTS `DoubleMajors`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `DoubleMajors`  AS  select `STUDENT`.`FNAME` AS `FNAME`,`STUDENT`.`LNAME` AS `LNAME`,`STUDENT_DOUBLE_MAJOR`.`SECOND_MAJOR` AS `SECOND_MAJOR` from (`STUDENT` join `STUDENT_DOUBLE_MAJOR` on(`STUDENT`.`STUDENT_ID` = `STUDENT_DOUBLE_MAJOR`.`STUDENT_ID`)) ;

-- --------------------------------------------------------

--
-- Structure for view `GraduateInfo`
--
DROP TABLE IF EXISTS `GraduateInfo`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `GraduateInfo`  AS  select `STUDENT`.`FNAME` AS `FNAME`,`STUDENT`.`LNAME` AS `LNAME`,`STUDENT`.`GPA` AS `GPA`,`GRADUATE`.`GRAD_DATE` AS `GRAD_DATE`,`GRADUATE_JOB`.`JOB_TYPE` AS `JOB_TYPE`,`GRADUATE_JOB`.`JOB_POSITION` AS `JOB_POSITION` from ((`GRADUATE` join `STUDENT` on(`GRADUATE`.`STUDENT_ID` = `STUDENT`.`STUDENT_ID`)) join `GRADUATE_JOB` on(`GRADUATE`.`STUDENT_ID` = `GRADUATE_JOB`.`STUDENT_ID`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `roster`  AS  select `STUDENT`.`FNAME` AS `FIRST NAME`,`STUDENT`.`LNAME` AS `LAST NAME`,`STUDENT`.`STUDENT_ID` AS `ID` from `STUDENT` ;

-- --------------------------------------------------------

--
-- Structure for view `StudentAdvisor`
--
DROP TABLE IF EXISTS `StudentAdvisor`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `StudentAdvisor`  AS  select `STUDENT`.`STUDENT_ID` AS `STUDENT_ID`,`STUDENT`.`FNAME` AS `FNAME`,`STUDENT`.`LNAME` AS `LNAME`,`STUDENT`.`GPA` AS `GPA`,`STUDENT_ADVISOR`.`ADVISOR_FNAME` AS `ADVISOR_FNAME`,`STUDENT_ADVISOR`.`ADVISOR_LNAME` AS `ADVISOR_LNAME` from (`STUDENT` join `STUDENT_ADVISOR` on(`STUDENT`.`STUDENT_ID` = `STUDENT_ADVISOR`.`STUDENT_ID`)) order by `STUDENT`.`LNAME` ;

-- --------------------------------------------------------

--
-- Structure for view `StudentInternships`
--
DROP TABLE IF EXISTS `StudentInternships`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ahanks`@`localhost` SQL SECURITY DEFINER VIEW `StudentInternships`  AS  select `STUDENT`.`FNAME` AS `FNAME`,`STUDENT`.`LNAME` AS `LNAME`,`STUDENT_INTERNSHIP`.`INTERNSHIP_TYPE` AS `INTERNSHIP_TYPE`,`STUDENT_INTERNSHIP`.`INTERNSHIP_POSITION` AS `INTERNSHIP_POSITION`,`INTERNSHIP_LOCATION`.`CITY` AS `INTERNSHIP CITY`,`INTERNSHIP_LOCATION`.`STATE` AS `INTERNSHIP STATE` from ((`STUDENT` join `STUDENT_INTERNSHIP` on(`STUDENT`.`STUDENT_ID` = `STUDENT_INTERNSHIP`.`STUDENT_ID`)) join `INTERNSHIP_LOCATION` on(`STUDENT_INTERNSHIP`.`INTERNSHIP_ID` = `INTERNSHIP_LOCATION`.`INTERNSHIP_ID`)) ;
